...
public class droneDataSimulator implements Runnable{
    ...
    //Constructor
    public droneDataSimulator(String p){
        this.dataPath = p;
    }
                ...
                if(nDronesProcessed < 4)
                    ...
                    for(int j=0; j<droneSquad.length; j++){
                        ...
                        for(int k=1; k<droneSquad[j].length; k++){
                            message += droneSquad[j][k];
                            if(k < droneSquad[j].length-1)
                                message += "><";
                        }
                        
                        //Envio a kafka
                        producer.send(new ProducerRecord<String, String>("droneMonitoring", Integer.toString(i+j), message));
                        message = "";
                        Thread.sleep(250);
                    }
    ...
    public static void main(String[] args) {
        ...
        threadPool.execute(new droneDataSimulator("..\\InputData\\Squad1.txt"));
        //threadPool.execute(new droneDataSimulator("..\\InputData\\Squad2.txt"));
        //threadPool.execute(new droneDataSimulator("..\\InputData\\Squad3.txt"));
